...
img <- image_read("Images/correlation_matrix_200.png")
...
All categorial varibles are composed of two results, so they are converted with 0 or 1. The Class category, on the other hand, is divided into 2 categories as it is composed of 3 possible results.
...
```{r}
AS_CTT_model <- glm(Satisfaction ~ Gate.Location + On.board.Service + Baggage.Handling + Check.in.Service + Online.Boarding + Customer.Type + Type.of.Travel + Class , data = train, family = binomial)    

summary(AS_CTT_model)
```

Below the execution of the test set with the current model:
```{r}
AS_CTT_model_probs <- predict(AS_CTT_model, testX, type = "response")

AS_CTT_model_roc <- roc(test$Satisfaction ~ AS_CTT_model_probs, plot=TRUE, print.auc=TRUE)
```
Returns all information for the classification threshold of 0.5.
```{r}
coords(AS_CTT_model_roc, x=0.5, ret="all")
```
Returns all information for the classification usign the best threshold.
```{r}
coords(AS_CTT_model_roc, x="best", ret="all")
```
...
effect_gate <- Effect("Gate.Location", AS_CTT_model)
effect_onboard <- Effect("On.board.Service", AS_CTT_model)
effect_checkin <- Effect("Check.in.Service", AS_CTT_model)
effect_Baggage <- Effect("Baggage.Handling", AS_CTT_model)
effect_Boarding <- Effect("Online.Boarding", AS_CTT_model)

effect_Customer.Type <- Effect("Customer.Type", AS_CTT_model)
effect_Type.of.Travel <- Effect("Type.of.Travel", AS_CTT_model)
effect_Boarding <- Effect("Class", AS_CTT_model)
...
```{r}
AS_CTT_CC_model <- glm(Satisfaction ~  Gate.Location + On.board.Service + Baggage.Handling + Check.in.Service + Online.Boarding + Customer.Type + Type.of.Travel + Class + Seat.Comfort + Leg.Room.Service + Cleanliness, data = train, family = binomial)    

summary(AS_CTT_CC_model)
```

Below the execution of the test set with the current model:
```{r}
AS_CTT_CC_model_probs <- predict(AS_CTT_CC_model, testX, type = "response")

AS_CTT_CC_model_roc <- roc(test$Satisfaction ~ AS_CTT_CC_model_probs, plot=TRUE, print.auc=TRUE)
```
Returns all information for the classification threshold of 0.5.
```{r}
coords(AS_CTT_CC_model_roc, x=0.5, ret="all")
```
Returns all information for the classification usign the best threshold.
```{r}
coords(AS_CTT_CC_model_roc, x="best", ret="all")
```

```{r, echo=FALSE}
effect_gate <- Effect("Gate.Location", AS_CTT_CC_model)
effect_onboard <- Effect("On.board.Service", AS_CTT_CC_model)
effect_checkin <- Effect("Check.in.Service", AS_CTT_CC_model)
effect_Baggage <- Effect("Baggage.Handling", AS_CTT_CC_model)
effect_Boarding <- Effect("Online.Boarding", AS_CTT_CC_model)

effect_Seat.Comfort <- Effect("Seat.Comfort", AS_CTT_CC_model)
effect_Leg.Room.Service <- Effect("Leg.Room.Service", AS_CTT_CC_model)
effect_Cleanliness <- Effect("Cleanliness", AS_CTT_CC_model)

effect_Customer.Type <- Effect("Customer.Type", AS_CTT_CC_model)
effect_Type.of.Travel <- Effect("Type.of.Travel", AS_CTT_CC_model)
effect_Boarding <- Effect("Class", AS_CTT_CC_model)
...
```{r}
AS_CTT_OBS_model <- glm(Satisfaction ~Gate.Location + On.board.Service + Baggage.Handling + Check.in.Service + Online.Boarding+ Customer.Type + Type.of.Travel + Class + Food.and.Drink + In.flight.Service + In.flight.Wifi.Service + In.flight.Entertainment , data = train, family = binomial)    

summary(AS_CTT_OBS_model)
```

Below the execution of the test set with the current model:
```{r}
AS_CTT_OBS_model_probs <- predict(AS_CTT_OBS_model, testX, type = "response")

AS_CTT_OBS_model_roc <- roc(test$Satisfaction ~ AS_CTT_OBS_model_probs, plot=TRUE, print.auc=TRUE)
```
Returns all information for the classification threshold of 0.5.
```{r}
coords(AS_CTT_OBS_model_roc, x=0.5, ret="all")
```
Returns all information for the classification usign the best threshold.
```{r}
coords(AS_CTT_OBS_model_roc, x="best", ret="all")
```


```{r, echo=FALSE}
effect_gate <- Effect("Gate.Location", AS_CTT_OBS_model)
effect_onboard <- Effect("On.board.Service", AS_CTT_OBS_model)
effect_checkin <- Effect("Check.in.Service", AS_CTT_OBS_model)
effect_Baggage <- Effect("Baggage.Handling", AS_CTT_OBS_model)
effect_Boarding <- Effect("Online.Boarding", AS_CTT_OBS_model)


effect_Customer.Type <- Effect("Customer.Type", AS_CTT_OBS_model)
effect_Type.of.Travel <- Effect("Type.of.Travel", AS_CTT_OBS_model)
effect_Boarding <- Effect("Class", AS_CTT_OBS_model)

effect_Food.and.Drinkt <- Effect("Food.and.Drink", AS_CTT_OBS_model)
effect_In.flight.Service <- Effect("In.flight.Service", AS_CTT_OBS_model)
effect_In.flight.Wifi.Service <- Effect("In.flight.Wifi.Service", AS_CTT_OBS_model)
effect_In.flight.Entertainment <- Effect("In.flight.Entertainment", AS_CTT_OBS_model)
...
* `AS_CTT_model`: Airport Services + Customer and Travel Type.

* `AS_CC_model`:  Airport Services + Comfort and Cleanliness.

* `AS_OBS_models`: Airport Services + On-board Services.

* `AS_CTT_CC_model`: Airport Services + Customer and Travel Type + Comfort and Cleanliness.

* `AS_CTT_OBS_model`: Airport Services + Customer and Travel Type + On-board Services.
...
model_data <- data.frame(
  Model = c("AS_CTT_model", 
            "TAS_CC_model", 
            "AS_OBS_models", 
            "AS_CTT_CC_model", 
            "AS_CTT_OBS_model", 
            "Complete_model"),
  AIC = c(AIC(AS_CTT_model), AIC(AS_CC_model), AIC(AS_OBS_model), AIC(AS_CTT_CC_model),AIC(AS_CTT_OBS_model),AIC(complete_model)),
  AUC = c(auc(AS_CTT_model_roc), auc(AS_CC_model_roc),auc(AS_OBS_model_roc),auc(AS_CTT_CC_model_roc),auc(AS_CTT_OBS_model_roc) ,auc(complete_model_roc)),
...
models_single <- data.frame(
  Model = c("Customer and Travel Type", "Time and Distance", "Convenience and Booking Services", 
            "On-board Services", "Comfort and Cleanliness", "Airport Services", "Demographic variables"),
  AIC = c(AIC(Customer_and_Travel_Type_model), AIC(Time_and_Distance_model), AIC(Convenience_and_Booking_Services_model), AIC(On_board_Services_model), AIC(Comfort_and_Cleanliness_model), AIC(Airport_Services_model), AIC(Demographic_variables_model)),
  AUC = c(auc(Customer_and_Travel_Type_model_roc), auc(Time_and_Distance_model_roc), auc(Convenience_and_Booking_Services_model_roc),auc(On_board_Services_model_model_roc), auc(Comfort_and_Cleanliness_model_roc), auc(Airport_Services_model_roc), auc(Demographic_variables_model_roc))
)

models_combined <- data.frame(
  Model = c("AS_CTT_model", "TAS_CC_model", "AS_OBS_models", 
            "AS_CTT_CC_model", "AS_CTT_OBS_model", "Complete_model"),
  AIC = c(AIC(AS_CTT_model), AIC(AS_CC_model), AIC(AS_OBS_model), AIC(AS_CTT_CC_model),AIC(AS_CTT_OBS_model),AIC(complete_model)),
  AUC = c(auc(AS_CTT_model_roc), auc(AS_CC_model_roc),auc(AS_OBS_model_roc),auc(AS_CTT_CC_model_roc),auc(AS_CTT_OBS_model_roc) ,auc(complete_model_roc))
)

models_single$Type <- "Single"
models_combined$Type <- "Combined"

data_to_plot <- rbind(models_single, models_combined)

data_to_plot$Model <- factor(data_to_plot$Model, 
                             levels = data_to_plot$Model[order(data_to_plot$AUC, -data_to_plot$AIC)])


p1 <- ggplot(data_to_plot, aes(x = Model, y = AIC, group = Type, color = Type)) +
  geom_bar(stat = "identity", aes(fill = Type), position = position_dodge(width = 0.8)) +
  geom_line(aes(y = AIC, group = 1), color = "black", size = 1) +
  geom_point(aes(y = AIC), color = "black", size = 2) +
  labs(title = "AIC Comparison between Single and Combined Models", x = "Model", y = "AIC") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

p2 <- ggplot(data_to_plot, aes(x = Model, y = AUC, group = Type, color = Type)) +
  geom_bar(stat = "identity", aes(fill = Type), position = position_dodge(width = 0.8)) +
  geom_line(aes(y = AUC, group = 1), color = "black", size = 1) +
  geom_point(aes(y = AUC), color = "black", size = 2) +
  labs(title = "AUC Comparison between Single and Combined Models", x = "Model", y = "AUC") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
Here we can see how the results of the models improve by combining the individual models together.
For the AIC, we see that combining the models decreases the value of the AIC. The best model is the one that combines the individual models.
For the auc on the other hand we can see the same improvement, the combination of the models leads to an increase in the final value and thus to a better result for the final prediction.

Final conclusions will be included in the final chapter.
...
Shrinkage methods are widely used in statistical modelling to reduce overfitting and improve model performance, especially when dealing with a large number of predictors. These methods work by applying a regularisation technique to reduce coefficient estimates towards zero.

Bias: when the model is too simple, it can under-fit the data, causing a high bias. Narrowing the coefficients towards zero increases the bias.
Variance: When the model is too complex, it can over-fit the data, capturing noise and irrelevant patterns, resulting in high variance. Reducing the coefficients reduces variance by discouraging complex models with high coefficients.
The goal is to find the optimal balance between bias and variance that minimises the mean square error. The mean square error is the sum of the squared differences between observed and predicted values.

The most common shrinkage methods:

* ridge regression

* lasso

Ridge regression adds a penalty to the sum of the squared coefficients (L2 penalty). It reduces the coefficients but never completely zeroes them, which means that all predictors are retained in the model.

Lasso adds a penalty to the sum of the absolute values of the coefficients (penalty L1). In contrast to the ridge regression, Lasso can reduce some coefficients to exactly zero, effectively making a selection of variables by excluding unimportant predictors.
...
We use Lasso to evaluate the best parameters. We use cross validation to calculate the best lambda value to be used in the model.
The Lasso model is based on calculating the sum of the absolute values of the coefficients.
...
These results show us which are the best predictors according to Lasso. We have 3 types of results: 

* Positive values: they influence improving the final prediction.
* Negative values: affect worsening the final prediction.
* Neutral values: do not affect the final result.

The most important predictors to be selected, considering the coefficients with the highest absolute values, are:

* Customer.TypeReturning (significant positive influence)

* Type.of.TravelBusiness (positive influence)

* Cleanliness (positive influence)

* In.flight.Wifi.Service (positive influence)

* Baggage.Handling (positive influence)

* Type.of.TravelPersonal (significant negative influence)

* Leg.Room.Service (negative influence)

* On.board.Service (negative influence)

* Food.and.Drink (negative influence)

These predictors are the most influential and should be prioritised when building a model or in further analyses.
...
```{r}
lasso_regression_model <- glm(Satisfaction ~ Customer.Type + Type.of.Travel + Cleanliness + In.flight.Wifi.Service + Baggage.Handling + Leg.Room.Service + On.board.Service + Food.and.Drink , data = train, family = binomial)
summary(lasso_regression_model)
```

From this model we can see that the best predictors belong to four specific classes: Customer and Travel Type, Comfort and Cleanliness, Airport Services and On-board services.
...
I will now implement the ridge model to evaluate the best predictors. I will use cross validation to find the best lambda value. The procedure is very similar to the lasso model but we change the value of alpha from 1 to zero. 
Ridge is based on calculating the sum of the squares of the coefficients.
...
```
Ridge regression does not select a subset of predictors.
Ridge regression includes all predictors p but all coefficients shrink towards zero.
The idea is that reducing the coefficients reduces the complexity of the model and reduces the risk of overfitting, but without completely eliminating any variables.
...
The predictors that seem to have the greatest impact on the dependent variable in the Ridge model are:

* Customer.TypeReturning (positive)

* Customer.TypeFirst-time (negative)

* Type.of.TravelPersonal (negative)

* ClassEconomy (negative)

* Check.in.Service (positive)

* Online.Boarding (positive)

* Seat.Comfort (positive)

* Ease.of.online.Booking (negative)

* Leg.Room.Service (positive)

* In.flight.Wifi.Service (positive)
...
Roc function reports the following results with a threshold of 0.5:
```{r}
coords(ridge_model_roc, x=0.5, ret="all")
```


And the following considering the best threshold:
```{r}
coords(ridge_model_roc, x="best", ret="all")
...
We can also try to fit a logistic regression model choosing the predictors suggested by Ridge

```{r}
ridge_regression_model <- glm(Satisfaction ~ Customer.Type + Type.of.Travel + Class + Check.in.Service + Online.Boarding + Seat.Comfort+ Ease.of.Online.Booking + Leg.Room.Service + In.flight.Wifi.Service + Cleanliness , data = train, family = binomial)
...
Finally, let us do a discriminant analysis using two techniques: 

* Linear Discriminant Analysis (LDA) 

* Quadratic Discriminant Analysis (QDA). 

These are statistical techniques used for classification. 
LDA assumes that the predictor variables (features) are normally distributed within each class and that all classes share the same covariance matrix. The objective of LDA is to find a linear combination of variables that maximises the separation between classes.

QDA is similar to LDA, but relaxes the assumption of the common covariance matrix. In QDA, each class can have its own covariance matrix.
This allows QDA to have a quadratic decision frontier, instead of a linear one.


Let us apply the two techniques to the 'Best_Model' model implemented above.
...
spiegari di nuovo delle classi singole + quelle unite

stepwise forse non serve

ridge e lasso mostrare come sono simili ai modelli precedenti
